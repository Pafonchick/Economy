...
const editorSections = {
  ...
  grp: { title: 'ГРР', kicker: 'Геолого-разведочные работы', fields: [['Период ГРР','2026–2029','text'],['Сейсмика 3D, км²','420','number'],['Поисковые скважины','3','number'],['Разведочные скважины','6','number'],['Бюджет, млрд ₽','8,4','text'],['Источник','Проект ГРР','text',1]] },
...
function grrRowsMarkup() {
  ...
  return grrTables[grrView].rows.map(([number, name, unit, kind], rowIndex) => `<tr class="grr-row-${kind}"><td>${number}</td><th>${name}</th><td>${unit}</td><td class="grr-total">${grrTotal(values[rowIndex])}</td>${grrYears.map((year, yearIndex) => `<td><input class="grr-value" data-row="${rowIndex}" data-year="${year}" value="${values[rowIndex][yearIndex]}" aria-label="${name}, ${year}" readonly></td>`).join('')}</tr>`).join('');
}
function grrMappingRows() {
  ...
  return [['Нулевой',zero],['Рекомендуемый',assets],['Максимальный',assets]].map(([variant, list]) => list.map((asset, index) => {
    ...
    return `<tr><td>${index ? '' : `<b>${variant}</b>`}</td><td>${asset.license}</td><td>${asset.name}</td><td><select class="grr-sheet-select"><option>&lt;Отсутствует&gt;</option><option selected>${prefix}_${shortName}_${asset.license.match(/\d+/)?.[0] || asset.code}</option><option>Программа_${asset.code}</option></select></td><td><span class="grr-load-status ok">✓ Проверен</span></td></tr>`;
  }).join('')).join('');
}
function renderGrrWorkspace() {
  ...
  $('#editorFields').innerHTML = `<div class="grr-workspace">
    <section class="grr-source"><div class="grr-source-icon">XLSX</div><div><b>Программа геолого-разведочных работ</b><small id="grrFileName">${grrFileName}</small></div><button class="button button-dark" type="button" data-grr-upload>Загрузить программу ГРР</button></section>
    ...
    <aside class="grr-import ${grrImportOpen ? 'open' : ''}" aria-hidden="${!grrImportOpen}"><div class="grr-import-head"><div><span>Импорт Excel</span><h4>Настройка исходных данных для программы ГРР</h4><small>Для каждого варианта и комбинации ЛУ + месторождение выберите отдельный видимый лист.</small></div><button type="button" class="icon-button" data-grr-close aria-label="Закрыть">×</button></div><div class="grr-file-row"><input id="grrFileInput" type="file" accept=".xlsx,.xlsm,.xls" hidden><button class="button button-secondary" type="button" data-grr-file>Выбрать файл</button><span><b>${grrFileName}</b><small>Лист «Шаблон» не включён в список · доступно значение «Отсутствует»</small></span></div><div class="grr-validation"><span>✓ Таблица 1: строки 3–14</span><span>✓ Таблица 2: строки 17–42</span><span>✓ Таблица 3: строки 45–77</span><span>✓ Начальный год: 2026</span></div><div class="grr-map-scroll"><table class="grr-map"><thead><tr><th>Вариант</th><th>Лицензия</th><th>Месторождение</th><th>Наименование листа из файла</th><th>Статус загрузки</th></tr></thead><tbody>${grrMappingRows()}</tbody></table></div><div class="grr-import-foot"><span id="grrImportHint">Все строки прошли минимальную проверку. Данные можно перенести.</span><button class="button button-primary" type="button" data-grr-import>Загрузить данные</button></div></aside>
  </div>`;
...
function renderGeoWorkspace() {
  ...
  const wellRows = [['Нефтяной','11'],['Газовый','—'],['Нагнетательный','3'],['Водозаборный','1'],['Прочие','—']].map(([name,value]) => `<tr><th>${name}</th><td>скв.</td><td><input value="${value}" disabled></td><td>МЭР</td></tr>`).join('');
  const manualRows = [
  ...
  ].map(([group,name,unit,key,value,enabled]) => `<tr><td>${group}</td><th>${name}</th><td>${unit}</td><td><input class="geo-manual" name="${key}" value="${value}" inputmode="decimal" ${enabled ? '' : 'disabled'}></td><td><span class="geo-table-state ${enabled ? 'editable' : 'locked'}">${enabled ? 'Ручной ввод' : 'Заблокировано'}</span></td></tr>`).join('');
  $('#editorFields').innerHTML = `<div class="geo-workspace geo-table-workspace">
    <section class="geo-table-panel"><header><div><span>01 · РН-КИН</span><h4>Запасы на 01.01.2026</h4></div><button class="button button-secondary" type="button" data-geo-load="reserves">${geoLoadState.reserves === 'loading' ? 'Загрузка…' : 'Загрузить геологические данные'}</button></header><div class="geo-table-scroll"><table class="geo-data-table"><thead><tr><th>Показатель</th><th>Ед. изм.</th>${geoReserveGroups.map(group => `<th>${group.title}</th>`).join('')}</tr></thead><tbody>${reserveRows}</tbody></table></div></section>
    <section class="geo-table-panel"><header><div><span>02 · МЭР</span><h4>Фонд скважин на конец 2025 года</h4></div><button class="button button-secondary" type="button" data-geo-load="wells">${geoLoadState.wells === 'loading' ? 'Загрузка…' : 'Загрузить данные по фонду'}</button></header><div class="geo-table-scroll"><table class="geo-data-table geo-well-table"><thead><tr><th>Тип фонда</th><th>Ед. изм.</th><th>Значение</th><th>Источник</th></tr></thead><tbody>${wellRows}<tr class="total-row"><th>Всего в фонде</th><td>скв.</td><td>15</td><td>МЭР</td></tr></tbody></table></div></section>
    <section class="geo-table-panel"><header><div><span>03 · Настраиваемые параметры</span><h4>Коэффициенты и ремонты</h4></div><small>Метод OPEX: ${currentOpexMethod() === 'normative' ? 'нормативный' : currentOpexMethod() === 'tariff' ? 'нормативно-тарифный' : 'ресурсный'}</small></header><div class="geo-table-scroll"><table class="geo-data-table geo-manual-table"><thead><tr><th>Группа</th><th>Показатель</th><th>Ед. изм.</th><th>Значение</th><th>Доступность</th></tr></thead><tbody>${manualRows}</tbody></table></div></section>
  </div>`;
...
function renderOpexWorkspace() {
  ...
  const groups = resource ? [] : opexNormGroups.filter(group => !(tariff && group.id === 'electricity'));
  $('#editorFields').innerHTML = `<div class="opex-workspace"><div class="opex-method"><div><span>Метод с главного экрана</span><b>${method === 'normative' ? 'Нормативный' : tariff ? 'Нормативно-тарифный' : 'Ресурсный'}</b></div><div><span>Область настройки</span><b>${resource ? 'Параметры ресурсного метода' : 'Единая для всех вариантов'}</b></div></div>${resource ? renderResourceOpex() : `<div class="opex-norm-grid">${groups.map(opexNormTable).join('')}</div>${tariff ? renderEnergyProfiles() : ''}`}</div>`;
  $('#editorFields').classList.add('opex-editor-fields');
  if (tariff) recalcEnergy();
}
...
function techRowsMarkup() {
  ...
  return techMetrics.map(([option,metric,unit], row) => {
    const dimension = techView === 'cluster' ? (option === 'Развитие' && row % 2 === 0 ? 'Куст 1' : '—') : techView === 'benefit' ? (option === 'Развитие' ? 'Кд 0,2' : '—') : techView === 'combined' ? (option === 'Развитие' ? 'Куст 1 · Кд 0,2' : '—') : '—';
    return `<tr class="${option === 'ИТОГО' ? 'group-row' : ''}"><td>${option}</td><th>${metric}</th><td>${dimension}</td><td>${unit}</td><td>—</td>${emptyYearInputs(years,'tech-value',true)}</tr>`;
  }).join('');
}
function techWellTable() {
  ...
  return `<section class="tech-wells"><header><h4>Поскважинная информация</h4><button class="text-button" type="button" data-tech-wells-close>Закрыть</button></header><div class="model-table-scroll"><table class="model-table"><thead><tr><th>Скважина</th><th>Тип</th><th>Назначение</th><th>Дата ввода</th><th>Ед. изм.</th><th>Всего</th>${yearHeader(years)}</tr></thead><tbody><tr class="group-row"><th colspan="5">Добыча нефти</th><td>200</td>${years.map((y,i)=>`<td>${[27,33,22,18,16,14,12][i]||''}</td>`).join('')}</tr>${wells.map(([well,type,use,date,total,values])=>`<tr><th>${well}</th><td>${type}</td><td>${use}</td><td>${date}</td><td>тыс. т</td><td>${total}</td>${years.map((y,i)=>`<td>${values[i]||''}</td>`).join('')}</tr>`).join('')}</tbody></table></div></section>`;
}
function renderTechWorkspace() {
  const years = macroYears;
  $('#editorFields').innerHTML = `<div class="model-workspace tech-workspace"><div class="section-toolbar"><div class="segmented">${[['general','Общий вид'],['cluster','По кустам'],['benefit','По категории льгот'],['combined','Кусты и льготы']].map(([key,label])=>`<button type="button" data-tech-view="${key}" class="${techView===key?'active':''}">${label}</button>`).join('')}</div><div class="period-fields"><label>Период<select><option>По умолчанию</option><option>Задать период</option></select></label><label>Год начала<input value="2026"></label><label>Год окончания<input value="2046"></label></div><button class="button button-dark" type="button" data-tech-load>Загрузить технологические данные и ГТМ</button><button class="button button-secondary" type="button" data-tech-wells>Поскважинная информация</button></div><div class="model-table-scroll"><table class="model-table tech-table"><thead><tr><th>Опция</th><th>Показатель</th><th>${techView==='general'?'Настройка':techView==='benefit'?'Кд':'Куст / Кд'}</th><th>Ед. изм.</th><th>Итого</th>${yearHeader(years)}</tr></thead><tbody>${techRowsMarkup()}</tbody></table></div>${techWellsOpen ? techWellTable() : ''}</div>`;
...
function renderCapexWorkspace() {
  ...
  $('#editorFields').innerHTML = `<div class="model-workspace capex-workspace"><div class="section-toolbar"><div class="segmented"><button type="button" data-capex-view="bp" class="${capexView==='bp'?'active':''}">Капитальные вложения по БП</button><button type="button" data-capex-view="all" class="${capexView==='all'?'active':''}">БП и перспективы</button></div><div class="toolbar-actions"><button class="button button-secondary" type="button" data-capex-action="load">Загрузить стоимости по БП</button><button class="button button-secondary" type="button" data-capex-action="edit">Редактировать объект КВ</button><button class="button button-secondary" type="button" data-capex-action="drill">Рассчитать бурение, ЗБС и ОНВСС</button><button class="button button-dark" type="button" data-capex-action="future">Сформировать стоимость за периодом БП</button></div></div><div class="model-table-scroll"><table class="model-table capex-table"><thead><tr><th>№</th><th>Тип объекта / наименование</th><th>Признак</th><th>Ед. изм.</th><th>Всего</th>${yearHeader(capexYears)}</tr></thead><tbody><tr class="total-row"><td></td><th>КАПИТАЛЬНЫЕ ВЛОЖЕНИЯ, ВСЕГО</th><td>—</td><td>тыс. руб</td><td>510 000</td>${emptyYearInputs(capexYears,'capex-value',false,seed)}</tr>${rows.map(([number,name,flag,unit,kind],index)=>`<tr class="${kind}-row"><td>${number}</td><th>${name}</th><td>${flag}</td><td>${unit}</td><td>${kind==='object'?'510 000':'—'}</td>${emptyYearInputs(capexYears,'capex-value',false,kind==='object'?seed:[])}</tr>`).join('')}</tbody></table></div></div>`;
  $('#editorFields').classList.add('model-editor-fields');
}
...
function renderOtherWorkspace() {
  ...
  $('#editorFields').innerHTML = `<div class="other-workspace"><section class="other-settings"><header><span>01</span><div><h4>Прочие настройки</h4></div></header><div><label><span>Использовать ставку НДПИ на нефть (без льгот)</span><select class="other-setting" name="ndpi"><option${settings.ndpi==='Нет'?' selected':''}>Нет</option><option${settings.ndpi==='Да'?' selected':''}>Да</option></select></label><label><span>Учёт переноса убытка</span><select class="other-setting" name="loss"><option${settings.loss==='КГН'?' selected':''}>КГН</option><option${settings.loss==='Вне КГН'?' selected':''}>Вне КГН</option><option${settings.loss==='Не учитывать'?' selected':''}>Не учитывать</option></select></label><div class="other-period"><span>Период расчёта</span><b>2026–${otherYears().at(-1)}</b><small>из «Инициатора»</small></div></div></section><section class="other-impact"><header><div><span>02 · Экологические платежи</span><h4>Плата за негативное воздействие на окружающую среду</h4></div></header><div class="other-variants">${otherVariants.map(([key,label])=>`<button type="button" data-other-variant="${key}" class="${otherVariant===key?'active':''}">${label} вариант</button>`).join('')}</div>${otherTable()}</section></div>`;
...
function renderEditorSection(sectionKey) {
  ...
  $('#editorSectionTitle').textContent = section.title; $('#editorSectionKicker').textContent = section.kicker;
  ...
  const sourceLabels = { grp:'Программа ГРР · Excel', tech:'Технологические данные и ГТМ', geo:'БД РН-Альфа · РН-КИН', tax:'Налоговый блок шаблона ЭМ', capex:'БП · перспективные объекты', opex:'Метод с главного экрана', other:'Период из «Инициатора»' };
  $('.source-badge').textContent = sourceLabels[sectionKey] || 'Из шаблона ЭМ';
...
function updateSetupUi() {
  ...
  $('#modelsReadyText').textContent = `${readyCount} из ${assets.length} комбинаций готовы`;
...
function markParametersDirty(event) {
  ...
  if (event?.target?.name === 'opex') { opexChanged = true; toast('Метод OPEX изменён', 'Потребуется повторная настройка раздела «Операционные затраты»', 'i-alert'); }
